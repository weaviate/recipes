...
With the `1.23` release, it is easier to use Weaviate with many open-source large language models (LLMs) such as Llama2-70b, CodeLlama-34b or Mistral-7B-Instruct. This is made possible by the [generative-anyscale](/developers/weaviate/model-providers/anyscale/generative) module.
...
* Read more about the `generative-anyscale` module [here](/developers/weaviate/model-providers/anyscale/generative).
...
The [`nodes` endpoint](developers/weaviate/config-refs/nodes) can be used to output information about the nodes in your cluster.
