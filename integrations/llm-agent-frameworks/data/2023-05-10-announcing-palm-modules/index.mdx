...
<!-- :::info Modules have since been renamed
These modules have since been renamed to `text2vec-google` and `generative-google`. They were previously called `text2vec-palm` and `generative-palm` respectively.
::: -->
...
These modules are available to all Weaviate users as of **today**, with the release of Weaviate version `v1.19.1`. They integrate the new Vertex PaLM API with Weaviate, allowing you to hit the ground running straight away with the latest in LLM and [vector database](https://weaviate.io/blog/what-is-a-vector-database) technologies.
